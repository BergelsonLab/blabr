...
test_that("Reading and wrangling babar output files for measures", {
  ...
  test_inventory <- get_inventory(babar_df, minimum_count = 0)
  test_consonant_inventory <- get_consonant_inventory(babar_df,minimum_count = 0)
  test_canonical <- get_canonical_metrics(babar_df)
  test_all <- get_metrics_and_inventory(babar_df, minimum_count = 0)
  test_with_minimum <- get_metrics_and_inventory(babar_df, minimum_count = 5)
