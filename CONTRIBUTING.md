...
- If you are on a branch:
  ...
  - Merge branch into `main`. Create a merge commit, don’t squash, don’t
    fast-forward. Amend the merge commit’s message if an automatic one
    was used.
...
When renaming or removing functions, we hard-deprecate them - make them
defunct. Create a section for the next `blabr` version in `R/defunct.R`
and move the old function there. See that file for examples.
